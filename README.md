...
这是一个可复用的中文教学视频工程。第一集从零讲清楚 WorkBuddy 的界面、第一条任务指令和安全验收方法；后续各集只需要替换 `content/episode.json`、对应截图和当集的 WorkBuddy 实操录屏，即可继续套用同一套明亮鲜艳的 PPT、固定女声、字幕和动画版式。

GitHub 仓库保留模板源码、内容协议、时间轴元数据和制作文档。官方界面截图、固定音色参考、生成音频、PPT 与成片等二进制资产放在本次本地工程包中，不直接提交到仓库；解压工程包即可获得可完整重渲染的版本。
...
1. 复制一份 `content/episode.json`，改标题、要点、旁白、截图和录屏路径。
...
第一集使用知识库“男女访谈式”中的固定女声 `female_question_reference_999.wav`，通过 `Fun-CosyVoice3-0.5B` 生成。参考音色以 SHA-256 校验锁定，每段保留整句语流、自然停顿和句尾转折；单段对参考音色不低于 0.60，相邻段之间不低于 0.86，达不到就停止生成。
